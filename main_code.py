...
"""
!pip install finance-datareader optuna koreanize-matplotlib --quiet

import FinanceDataReader as fdr
import pandas as pd, numpy as np, matplotlib.pyplot as plt, koreanize_matplotlib, optuna
...
def objective(trial):
    ...
    vals = run_dyn(train_df, rf_train, **p,
                   cash0=CFG["initial_cash"], fee=CFG["fee"])
...
# ------------------------------- 5. Test 평가 ----------------------------
vals_test = run_dyn(test_df, rf_test, **best,
                    cash0=CFG["initial_cash"], fee=CFG["fee"])
days_test = (test_df["날짜"].iat[-1]-test_df["날짜"].iat[0]).days
sh, so = vec_stats(vals_test, rf_test)
print(f" Test → CAGR {cagr(vals_test, days_test):.4%} | Sharpe {sh:.3f} | Sortino {so:.3f}")

# ------------------------------- 6. 전체 구간 실행 ------------------------
vals_all = run_dyn(price, rf_daily, **best,
                   cash0=CFG["initial_cash"], fee=CFG["fee"])
